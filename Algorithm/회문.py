...
def isPalindrome(s, left, right, flag): # 회문 판별 함수
    while left < right: # 투 포인터
        if s[left] == s[right]: # 같다면 포인터 이동
            left += 1; right -= 1
        elif flag: # 한 문자를 삭제했는데 다른문자가 나온다면 회문도, 유사회문도 아니므로 2 반환
            return 2
        else:
            if isPalindrome(s, left+1, right, True) == 1: # 왼쪽 문자를 삭제하여 회문 판별 후 유사회문 가능하면 1 반환
                return 1
            else:
                return isPalindrome(s, left, right-1, True)  # 오른쪽 문자를 삭제하여 회문 판별 후 유사회문 가능하면 아래 리턴문을 통해 반환
            
    return 1 if flag else 0 # 삭제했었다면 1 반환, 그 자체로 회문이라면 0 반환

for _ in range(T):
    s = input()

    print(isPalindrome(s, 0, len(s)-1, False))
