...
for _ in range(T):
    ...
    applicant = sorted(applicant) # 서류심사 성적을 기준으로 정렬
    answer = 1 # 첫번째는 서류가 1등이므로 무조건 합격
    cut = applicant[0][1] # 첫번째의 면접을 컷으로 설정

    for i in range(1, len(applicant)):
        if applicant[i][1] < cut: # 면접점수가 컷보다 등수가 높다면 합격
            answer += 1
            cut = applicant[i][1] # 컷 점수 재설정
