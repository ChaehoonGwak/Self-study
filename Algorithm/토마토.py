...
def bfs():
    depth = [[0]*M for _ in range(N)] # 토마토가 모두 익을 때까지의 날짜를 구하기 위한 2차원리스트
    q = deque()
    
    for i in range(N):
        for j in range(M):
            if graph[i][j] == 1: # 익은 토마토를 큐에 삽입
                q.append([i, j])
    while q:
        y, x = q.popleft()
        
        for i in range(4): # 왼쪽, 오른쪽, 앞, 뒤 네 방향 인접한 토마토를 확인
            nx = x + dx[i]
            ny = y + dy[i]
            if 0 <= nx < M and 0 <= ny < N:
                if graph[ny][nx] == 0: # 인접한 토마토가 익지 않았다면
                    graph[ny][nx] = 1 # 익는다
                    depth[ny][nx] = depth[y][x]+1 # 토마토가 익기 전 날짜+1
                    q.append([ny, nx]) # 익은 토마토를 큐에 삽입
                    
    if 0 != sum(i.count(0) for i in graph): # 만약 0이 남아 있다면 토마토가 모두 익지는 못하는 상황
        print(-1) # -1을 출력
    else:
        print(max(map(max, depth))) # 모두 익은 상황이라면 익을때까지 걸린 최소 날짜를 출력
