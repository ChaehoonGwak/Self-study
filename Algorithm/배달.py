...
def dijkstra(start, road, distance):
    ...
    while q:
        ...
        if distance[now] < dist: # 이미 처리된 것이라면 무시
            continue
        
        for x in road:
            if x[0] == now: # 출발지가 현재 마을인 경우
                cost = dist + x[2]
                next = x[1]
                if cost < distance[next]: # 
                    distance[next] = cost
                    heapq.heappush(q, (cost, next))
            elif x[1] == now: # 도착지가 현재 마을인 경우
                cost = dist + x[2]
                prev = x[0]
...
def solution(N, road, K):
    answer = 0
    start = 1 # 시작 마을
    distance = [int(1e9)] * (N + 1) # 최단 거리 테이블 무한으로 초기화
    
    dijkstra(start, road, distance) # 다익스트라 알고리즘
    
    for x in distance:
        if x <= K: # 배달 거리가 K 이하인 마을 갯수 구함
            answer += 1
