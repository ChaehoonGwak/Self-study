...
def solution(k, dungeons):
    answer = -1
    cases = list(permutations(dungeons)) # 던전을 탐험하는 모든 순서의 조합
    
    for case in cases: # 모든 조합을 체크 - BRUTE FORCE
        point = k # 현재 남은 피로도
        play_dgs = 0 # 탐험한 던전의 횟수
        
        for c in case:
            least_point = c[0]; consume_point = c[1]
            if point >= least_point: # 최소 필요 피로도보다 현재 남은 피로도가 높다면 탐험
                point -= consume_point
                play_dgs += 1
                
        answer = max(answer, play_dgs) # 최댓값을 갱신
